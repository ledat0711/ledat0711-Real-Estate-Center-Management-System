...
export class NotificationListComponent implements OnInit {
  ...
  listDeletedNotifications: string = '';
  ...
  sendToDeleteGroupModal(): void {
    ...
    this.notificationService.findByListId(this.deleteIds).subscribe(data => {
      let i: number;
      let listDeletedNotifications: string = '';
      for (i = 0; i < data.length; i++) {
        numericalOrder = '' + (data.length - i);
        if (data.length == 1) {
          numericalOrder = '';
        }
        listDeletedNotifications = '<br>' + numericalOrder + '-' + data[i].title + listDeletedNotifications;
      }
      this.listDeletedNotifications = listDeletedNotifications + '<br>';
  ...
  delete(): void {
    this.notificationService.delete(this.deleteIds).subscribe(next => {
      this.toastrService.info('Xóa thành công '+this.deleteIds.length+' thông báo:' + this.listDeletedNotifications, 'Thông báo', {
        timeOut: 6000,
        disableTimeOut: true,
        closeButton:true,
        progressBar: true,
