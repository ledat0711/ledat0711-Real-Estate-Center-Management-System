...
public interface INotificationRepository extends JpaRepository<Notification, Long> {
     ...
     * Function: auto delete notifications with creation date before 3 months ago
     */
    @Modifying
    @Query(value = "UPDATE `notification` SET flag_delete = TRUE WHERE (DATE(posting_date) <= DATE_SUB(NOW(), INTERVAL 3 MONTH)) AND flag_delete = FALSE", nativeQuery = true)
    void autoDeleteNotification();
